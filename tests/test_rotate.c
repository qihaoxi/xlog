...
static void cleanup_test_dir(void) {
    ...
    char pattern[128];
    snprintf(pattern, sizeof(pattern), "%s*", TEST_BASE);

    int count;
    char **files = NULL;

    /* Simple cleanup - remove known patterns */
    char path[512];
    ...
    /* Remove dated files (try a range) */
    for (int d = 1; d <= 31; d++) {
        snprintf(path, sizeof(path), "%s/%s-20260209%s", TEST_DIR, TEST_BASE, TEST_EXT);
        xlog_remove(path);

        for (int seq = 1; seq <= 10; seq++) {
            snprintf(path, sizeof(path), "%s/%s-20260209-%02d%s",
                     TEST_DIR, TEST_BASE, seq, TEST_EXT);
            xlog_remove(path);
        }
    }

    /* Also clean up any files from different dates */
    for (int d = 1; d <= 28; d++) {
        snprintf(path, sizeof(path), "%s/%s-202602%02d%s", TEST_DIR, TEST_BASE, d, TEST_EXT);
        xlog_remove(path);
        for (int seq = 1; seq <= 10; seq++) {
            snprintf(path, sizeof(path), "%s/%s-202602%02d-%02d%s",
                     TEST_DIR, TEST_BASE, d, seq, TEST_EXT);
            xlog_remove(path);
        }
    }
}
